...
public class SolutionUseDp {
    ...
    // 2:58 下午	info
    //				解答成功:
    //				执行耗时:61 ms,击败了7.69% 的Java用户
    //				内存消耗:39 MB,击败了39.67% 的Java用户

    // TODO 由于数据样本的缘故,dp的表现反而不如记忆化搜索
     ...
     */
    private int process() {
        int[][] dp = new int[ringSize][key.length() + 1];
        for (int currentIndexOnKey = key.length() - 1; currentIndexOnKey >= 0; currentIndexOnKey--) {
            ...
            for (int currentIndexOnRing = ringSize - 1; currentIndexOnRing >= 0; currentIndexOnRing--) {
                ...
                for (Integer targetCharOnRingIndex : targetCharOnRingIndexList) {
                    ...
                    int rightTotalCost = rightCost + 1 + dp[targetCharOnRingIndex][currentIndexOnKey + 1];
                    int leftCost = Math.abs(ringSize - rightCost);
                    int leftTotalCost = leftCost + 1 + dp[targetCharOnRingIndex][currentIndexOnKey + 1];
                    minStep = Math.min(minStep, rightTotalCost);
                    minStep = Math.min(minStep, leftTotalCost);
                }
                dp[currentIndexOnRing][currentIndexOnKey] = minStep;
            }
        }
        return dp[0][0];
    }
