...
public class SolutionUseDp {
    ...
    public long process(int maxMinusHp, int monsterHp, int attackTimes) {
        long[][] dp = new long[attackTimes + 1][monsterHp + 1];
        dp[0][0] = 1;
            ...
            for (int leftHp = 0; leftHp <= monsterHp; leftHp++) {
                ...
                for (int currentMinusHp = 0; currentMinusHp <= maxMinusHp; currentMinusHp++) {
                    ...
                    if (leftHpAfterAttack > 0) {
                        ways += dp[leftAttackTime - 1][leftHpAfterAttack];
                    } else {
                        ways += (long)Math.pow(maxMinusHp + 1, leftAttackTime - 1);
                    }
                }
                dp[leftAttackTime][leftHp] = ways;
            }
        }
        return dp[attackTimes][monsterHp];
    }
     ...
     * @return 杀死怪兽的方案数
     */
    public long process2(int maxMinusHp, int monsterHp, int attackTimes) {
        long[][] dp = new long[attackTimes + 1][monsterHp + 1];
        dp[0][0] = 1;
        for (int leftAttackTime = 1; leftAttackTime <= attackTimes; leftAttackTime++) {
            //TODO 此处是为了在发动攻击后,剩余血量==0时,从此处取值
            dp[leftAttackTime][0] = (long)Math.pow(maxMinusHp + 1, leftAttackTime);

            for (int leftHp = 0; leftHp <= monsterHp; leftHp++) {
                long ways = 0;
                for (int currentMinusHp = 0; currentMinusHp <= maxMinusHp; currentMinusHp++) {
                    int leftHpAfterAttack = leftHp - currentMinusHp;
                    //TODO 此处如果是大于等于0,需要提前计算==0时的方案数
                    if (leftHpAfterAttack >= 0) {
                        ways += dp[leftAttackTime - 1][leftHpAfterAttack];
                    } else {
                        ways += (long)Math.pow(maxMinusHp + 1, leftAttackTime - 1);
