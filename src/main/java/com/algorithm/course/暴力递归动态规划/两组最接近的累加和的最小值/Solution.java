package com.algorithm.course.暴力递归动态规划.两组最接近的累加和的最小值;
...
public class Solution {
    ...
    public int towCloselySumReturnMin(int[] arr) {
        ...
        int aimNumber = sum >> 1;
        return process(arr, 0, aimNumber);
    }
     ...
     * @return 当前累计值
     */
    public int process(int[] arr, int index, int leftNumber) {

        if (index >= arr.length) {
            return 0;
        }
        // 分两种情况讨论,选择当前数字 和 不选择当前数字
        int minSum = process(arr, index + 1, leftNumber);

        // 因为有次条件限制,累加和一定不会超过aimNumber
        if (arr[index] <= leftNumber) {
            int afterChooseSum = arr[index] + process(arr, index + 1, leftNumber - arr[index]);
