...
public class SolutionUseCache {
    ...
    // 11:51 上午	info
    //				解答成功:
    //				执行耗时:250 ms,击败了6.24% 的Java用户
    //				内存消耗:169.6 MB,击败了5.04% 的Java用户
    ...
    public boolean isInterleave(String s1, String s2, String s3) {
        ...
        this.s2 = s2;
        this.s3 = s3;
        int[][][][] cache = new int[s1.length() + 1][s2.length() + 1][s3.length() + 1][2];

        return process(0, 0, 0, 0, cache) || process(0, 0, 0, 1, cache);
    }
     ...
     * @param preChoose 上一次选择是匹配了1串还是2串
     * @return true:是交替串,false 不是交替串
     */
    private boolean process(int index1, int index2, int index3, int preChoose, int[][][][] cache) {
        // 目标串匹配结束
        if (index3 >= s3.length()) {
            return index1 >= s1.length() && index2 >= s2.length();
        }

        if (cache[index1][index2][index3][preChoose] != 0) {
            return cache[index1][index2][index3][preChoose] > 0;
        }
        ...
        if (preChoose == 0) {
            ...
            for (int i = 1; i <= length; i++) {
                boolean result = process(index1, index2 + i, index3 + i, 1, cache);
                if (result) {
                    cache[index1][index2][index3][preChoose] = 1;
                    return true;
                }
            }
        } else {
            ...
            for (int i = 1; i <= length; i++) {
                boolean result = process(index1 + i, index2, index3 + i, 0, cache);
                if (result) {
                    cache[index1][index2][index3][preChoose] = 1;
                    return true;
                }
