...
public class SolutionUseDpCompressSlopeOptimize {
    ...
    public long process(int maxMinusHp, int monsterHp, int attackTimes) {
        ...
        for (int leftAttackTime = 1; leftAttackTime <= attackTimes; leftAttackTime++) {
            dp[target][0] = (long)Math.pow(maxMinusHp + 1, leftAttackTime);
            for (int leftHp = 1; leftHp <= monsterHp; leftHp++) {
                //TODO 此处用到leftHp-1,结果可能为0,所以需要提前计算好dp[target][0]的值
                long ways = dp[target][leftHp - 1] + dp[source][leftHp];
                // 如果当前血量减去最大减血量再减去1仍旧大于0,说明多加了一个在表格内的方案数
                int leftHpAfterAttack = leftHp - maxMinusHp - 1;
                // 此处可以用>=0,已经计算好饿了等于0是的方案数
