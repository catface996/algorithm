...
public class SolutionUseCache {
    ...
    public int calculateEarliestTime(int[] coffeeMachine, int peopleNum, int washPeriod, int evaporatePeriod) {
        ...
        // 初始化缓存,用0表示未处理
        int[][] cache = initCache(drinkOffArr);
        // 计算所有杯子都变干净的最早时间
        return process(drinkOffArr, 0, 0, cache);
    }
     ...
     * @param cache         对当前要处理的杯子,洗碗机下次可用时间做缓存
     * @return 所有杯子全部变干净的最早时间
     */
    private int process(int[] drinkOffArr, int next, int startWashTime, int[][] cache) {
        // 所有的杯子都已经处理,洗碗机下次可用的时间,即之前的杯子全部变干净的最早时间
        if (next >= drinkOffArr.length) {
            return startWashTime;
        }
        // 如果命中缓存,直接返回
        int ans = cache[next][startWashTime];
        if (ans != 0) {
            return ans;
        }
        ...
        int leftTimeAfterWash = process(drinkOffArr, next + 1, washOffTime, cache);
        ...
        int leftTimeAfterEvaporate = process(drinkOffArr, next + 1, startWashTime, cache);
        int earliestTime2 = Math.max(evaporateOffTime, leftTimeAfterEvaporate);
        // 做完选择后,取当前选择和选择后的最大值作为结束时间
        // 两种方案中,选择最早结束的时间作为最优时间
        int earliestTime = Math.min(earliestTime1, earliestTime2);
        cache[next][startWashTime] = earliestTime;
        return earliestTime;
    }
     ...
     * @return 最晚喝完咖啡的时间
     */
    private int getLastDrinkOffTime(int[] drinkOffArr) {
        int lastDrinkOffTime = 0;
        for (int j : drinkOffArr) {
            lastDrinkOffTime = Math.max(lastDrinkOffTime, j);
        }
        return lastDrinkOffTime;
    }

    /**
     * 初始化缓存,最早结束时间一定不会为0,所以可以用0表示未加入缓存
     *
     * @param drinkOffArr 每个人喝完咖啡的时间列表
     * @return 缓存
     */
    private int[][] initCache(int[] drinkOffArr) {
        return new int[drinkOffArr.length][getLastDrinkOffTime(drinkOffArr) + Math.max(washPeriod,
            evaporatePeriod)];
    }
    ...
    public static class TestClass {
        ...
        public void test1() {
            ...
            int earliestTime = solutionUseCache.calculateEarliestTime(machineArr, peopleNum, washPeriod,
                evaporatePeriod);
