...
public class SolutionUseCache {
    ...
    //2:45 下午	info
    //				解答成功:
    //				执行耗时:11 ms,击败了77.33% 的Java用户
    //				内存消耗:39 MB,击败了36.44% 的Java用户
    ...
    public int findRotateSteps(String ring, String key) {
        ...
        int[][] cache = new int[ringSize][key.length()];
        for (int[] ints : cache) {
            Arrays.fill(ints, -1);
        }
        return process(0, 0, cache);
    }
     ...
     * @return 最小代价
     */
    private int process(int currentIndexOnRing, int currentIndexOnKey, int[][] cache) {
        if (currentIndexOnKey >= key.length()) {
            // key已经全部打印完成
            return 0;
        }
        if (cache[currentIndexOnRing][currentIndexOnKey] != -1) {
            return cache[currentIndexOnRing][currentIndexOnKey];
        }
        ...
        for (Integer targetCharOnRingIndex : targetCharOnRingIndexList) {
            ...
            int rightTotalCost = rightCost + 1 + process(targetCharOnRingIndex, currentIndexOnKey + 1, cache);
            int leftCost = Math.abs(ringSize - rightCost);
            int leftTotalCost = leftCost + 1 + process(targetCharOnRingIndex, currentIndexOnKey + 1, cache);
            minStep = Math.min(minStep, rightTotalCost);
            minStep = Math.min(minStep, leftTotalCost);
        }
        cache[currentIndexOnRing][currentIndexOnKey] = minStep;
        return minStep;
    }
