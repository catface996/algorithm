...
import com.algorithm.course.有序表.sbt.impl.Code01_SizeBalancedTreeMap.SBTNode;
...
public class PrintSBTNode {
    ...
    private static int getMaxDepth(SBTNode x) {
        ...
        int leftDepth = getMaxDepth(x.l);
        int rightDepth = getMaxDepth(x.r);
    ...
    private static void process(SBTNode x, int start, int end, int depth, char[][] bitmap) {
        ...
        process(x.l, start, middle, depth + 1, bitmap);
        process(x.r, middle, end, depth + 1, bitmap);
    }
