package course.暴力递归动态规划.币值组合III;
...
public class SolutionUseCache {
    ...
    public int calculateWays(int[] moneyArr, int aimAmount) {
        ...
        int[][] cache = ArrayUtil.initCache(moneyArr.length, aimAmount + 1, -1);
        return process(moneyInfos, 0, aimAmount, cache);
    }
     ...
     * @return 有效的币值组合数量
     */
    public int process(MoneyInfo[] moneyInfos, int currentIndex, int leftAmount, int[][] cache) {
        // 剩余目标金额为0,发现一种有效组合
        if (leftAmount == 0) {
            return 1;
        }
        ...
        for (int chooseNum = 0; chooseNum <= currentMoney.num; chooseNum++) {
            ...
            ways += process(moneyInfos, currentIndex + 1, leftAmountAfterChoose, cache);
        }
    ...
    public static class TestClass {
        ...
        public void test2() {
            for (int i = 0; i < 1000; i++) {
                ...
                int ways = solutionUseCache.calculateWays(moneyArr, aimAmount);
