...
class Solution2UseCache {
    ...
    public int maxSubArray(int[] nums) {
        ...
        for (int end = 0; end < nums.length; end++) {
            // 已end为结尾的所有数组中的最大值
            max = Math.max(max, maxByEnd(nums, end, cache));
        }
        return max;
    }
     ...
     * @return 以end为结尾的最大值
     */
    public int maxByEnd(int[] nums, int end, int[] cache) {
        if (end == 0) {
            return nums[0];
        }
        int ans = cache[end];
        ...
        int max = maxByEnd(nums, end - 1, cache);
        ans = Math.max(max + nums[end], nums[end]);
        cache[end] = ans;
        return ans;
    }
