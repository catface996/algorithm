...
public class SolutionUseDpSlopeOptimize {
    ...
    public int process(int[] moneyValueArr, int aimAmount) {
        ...
        for (int current = moneyValueArr.length - 1; current >= 0; current--) {
            for (int leftAimAmount = 0; leftAimAmount <= aimAmount; leftAimAmount++) {
                int minNum = dp[current + 1][leftAimAmount];
                int lessOneCurrentMoneyAmount = leftAimAmount - moneyValueArr[current];
                if (lessOneCurrentMoneyAmount >= 0 && dp[current][lessOneCurrentMoneyAmount] != Integer.MAX_VALUE) {
                    minNum = Math.min(minNum, dp[current][lessOneCurrentMoneyAmount] + 1);
                }
    ...
    public static class TestClass {
        ...
        public void test2() {
            for (int i = 0; i < 100; i++) {
                ...
                int minNum2 = Code02_MinCoinsNoLimit.minCoins(moneyValueArr, aimAmount);
                if (minNum != minNum2) {
                    System.out.println("目标金额:" + aimAmount);
                }
            }
