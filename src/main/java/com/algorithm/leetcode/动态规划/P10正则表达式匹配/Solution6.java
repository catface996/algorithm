...
public class Solution6 {
    ...
    public boolean isMatch(String s, String p) {
        ...
        setCache(-1, -1, true, ansMap);
        int pRight = -1 + 2;
        while (pRight < p.length() && p.charAt(pRight) == '*') {
            setCache(-1, pRight - 1, true, ansMap);
            pRight += 2;
        }
        boolean ans;
        for (int sRight = -1; sRight < s.length(); sRight++) {
            for (pRight = -1; pRight < p.length(); pRight++) {
                ans = isMatch(s, p, sRight, pRight, ansMap);
                setCache(sRight, pRight, ans, ansMap);
            }
        }
        return getCache(s.length() - 1, p.length() - 1, ansMap);
    }
    ...
    public boolean isMatch(String s, String p, int sRight, int pRight, boolean[][] ansMap) {
        ...
        if (sRight == -1 && pRight >= 0) {
            if (p.charAt(pRight) == '*') {
                ...
                return getCache(sRight, pRight - 2, ansMap);
            }
            return false;
        }
        ...
        // 1和2合并
        if (s.charAt(sRight) == p.charAt(pRight) || p.charAt(pRight) == '.') {
            return getCache(sRight - 1, pRight - 1, ansMap);
        }
        // 3.如果pRight处字符是'*',
        if (p.charAt(pRight) == '*') {
            // 3.1 将pRight处的'*'与pRight-1处的字符合并为0个字符考虑
            if (getCache(sRight, pRight - 2, ansMap)) {
                return true;
            }
            // 3.2 将pRight处的'*'和pRight-1处的字符合并为1个字符考虑
            if (getCache(sRight, pRight - 1, ansMap)) {
                return true;
            }
            // 3.3 将pRight处的'*'和pRight-1处的字符合并为多个字符考虑,此时如果pRight-1处字符与sRight处字符匹配,方可进入下一轮匹配
            // 根据题设,pRight处是'*'时,pRight-1处一定有有效字符,严禁起见,可以判断pRight-1>=0
            if (p.charAt(pRight - 1) == '.' || p.charAt(pRight - 1) == s.charAt(sRight)) {
                return getCache(sRight - 1, pRight, ansMap);
            }
        }
        // 以上条件均不符合,一定是不匹配的
        return false;
    ...
    public void setCache(int sRight, int pRight, boolean ans, boolean[][] ansMap) {
        sRight++;
        pRight += 2;
        ansMap[sRight][pRight] = ans;
    }

    public boolean getCache(int sRight, int pRight, boolean[][] ansMap) {
        sRight++;
        pRight += 2;
