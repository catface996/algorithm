...
public class SolutionUseCache {
    ...
    public int calculateWays(int[] moneyValueArr, int aimAmount) {
        ...
        cache = new int[moneyValueArr.length][aimAmount + 1];
        for (int[] ints : cache) {
            Arrays.fill(ints, -1);
        }
    ...
    public int process(int currentIndex, int leftAmount) {
        ...
        do {
            newLeftAmount = leftAmount - chooseNum * moneyValueArr[currentIndex];
            ways += process(currentIndex + 1, newLeftAmount);
            chooseNum++;
        } while (newLeftAmount >= 0);
        // 加入缓存
