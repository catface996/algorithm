package com.algorithm.course.暴力递归动态规划.咖啡机泡咖啡洗杯子;
...
public class SolutionUseDp {
    ...
    public int calculateEarliestTime(int[] coffeeMachine, int peopleNum, int washPeriod, int evaporatePeriod) {
        ...
        int[] drinkOffArr = lineUp(peopleNum);
        // 计算所有杯子都变干净的最早时间
        return process(drinkOffArr);
    }
     ...
     */
    private int process(int[] drinkOffArr) {

        int maxStartWashTime = getMaxStartWashTime(drinkOffArr);
        int[][] dp = new int[drinkOffArr.length + 1][maxStartWashTime + 1];
        for (int next = drinkOffArr.length - 1; next >= 0; next--) {
            for (int startWashTime = 0; startWashTime <= maxStartWashTime; startWashTime++) {
                ...
                if (washOffTime > maxStartWashTime) {
                    break;
                }
                int leftTimeAfterWash = dp[next + 1][washOffTime];
     ...
     */
    private int[] lineUp(int peopleNum) {
        // 初始化咖啡机
    ...
    /**
     * 获取最晚喝完咖啡的时间(即最晚喝到咖啡的时间)
     *
     * @param drinkOffArr 每个人喝完咖啡的时间列表
     * @return 最晚喝完咖啡的时间
     */
    private int getMaxStartWashTime(int[] drinkOffArr) {
        int lastDrinkOffTime = 0;
        for (int j : drinkOffArr) {
            lastDrinkOffTime = Math.max(lastDrinkOffTime, j) + washPeriod;
        }
        return lastDrinkOffTime;
    }
    ...
    public static class TestClass {
        ...
        public void test1() {
            ...
            int earliestTimeUseDp = solutionUseDp.calculateEarliestTime(machineArr, peopleNum, washPeriod,
                evaporatePeriod);
            SolutionUseCache solutionUseCache = new SolutionUseCache();
            int earliestTimeUseCache = solutionUseCache.calculateEarliestTime(machineArr, peopleNum, washPeriod,
                evaporatePeriod);
            System.out.println("所有杯子变干净的最早时间UseDp:" + earliestTimeUseDp);
            System.out.println("所有杯子变干净的最早时间UseCache:" + earliestTimeUseCache);
