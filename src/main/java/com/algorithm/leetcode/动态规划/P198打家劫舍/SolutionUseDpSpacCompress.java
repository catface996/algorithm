...
class SolutionUseDpSpacCompress {

    // 3:36 下午	info
    //				解答成功:
    //				执行耗时:0 ms,击败了100.00% 的Java用户
    //				内存消耗:35.5 MB,击败了96.14% 的Java用户
    ...
    private int processDp(int[] nums) {
        ...
        for (int current = nums.length - 1; current >= 0; current--) {
            ans = Math.max(current1, nums[current] + current2);
            current2 = current1;
            current1 = ans;
        }
        return ans;
    ...
    public static class TestClass {
        ...
        public void test1() {
            int[] nums = {1, 2, 3, 1};
            SolutionUseDpSpacCompress solution = new SolutionUseDpSpacCompress();
