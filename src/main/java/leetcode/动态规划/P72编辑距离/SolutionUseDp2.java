...
@Slf4j
public class SolutionUseDp2 {

    // 10:18 下午	info
    //					解答成功:
    //					执行耗时:6 ms,击败了87.64% 的Java用户
    //					内存消耗:38.1 MB,击败了96.72% 的Java用户
    ...
    private int process(String word1, String word2) {
        ...
        for (int l1 = 1; l1 < N; l1++) {
            for (int l2 = 1; l2 < M; l2++) {
                ...
                minCost = Math.min(dp[l1 - 1][l2], dp[l1][l2 - 1]) + 1;
                // 3.如果末尾字符相同,通过保留末尾构成目标字符串,则需要l1-1构成l2-1
                // 4.如果末尾字符不相同,通过替换末尾字符来构成目标字符串
                dp[l1][l2] = Math.min(minCost,
                    dp[l1 - 1][l2 - 1] + (word1.charAt(l1 - 1) == word2.charAt(l2 - 1) ? 0 : 1));
            }
        }
        return dp[N - 1][M - 1];
    }
    ...
    public static class TestClass {
        ...
        public void test1() {
            String word1 = "horse";
            String word2 = "ros";
            SolutionUseDp2 solution = new SolutionUseDp2();
