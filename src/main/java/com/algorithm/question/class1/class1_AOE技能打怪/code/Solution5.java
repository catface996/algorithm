...
public class Solution5 {
    ...
    public int minAoeNums(int[] x, int[] hp, int range) {
        ...
        //清空窗口内的怪兽,需要释放的技能次数
        int windowSum = 0;
        MyQueue hpWindow = new MyQueue(monsterNums);
        MyQueue iWindow = new MyQueue(monsterNums);
        do {
            if (hpWindow.isEmpty()) {
            ...
            if (!hpWindow.isEmpty()) {
                ...
                //    MessageFormat.format(reg, currentLeftI, targetI, currentRightI, -leftMonsterDieHp, ans));
            }
