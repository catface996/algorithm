...
class Solution {
    ...
    public int[][] merge(int[][] intervals) {
        ...
        int ansNum = 0;
        int[][] endStartFirst = mergeSortByEndStart(intervals, 0, intervals.length - 1);
        int minStart = endStartFirst[0][0];
        int maxEnd = endStartFirst[0][1];

        for (int i = 1; i < endStartFirst.length; i++) {
            if (endStartFirst[i][0] > maxEnd) {
                temp[ansNum][0] = minStart;
                temp[ansNum][1] = maxEnd;
                minStart = endStartFirst[i][0];
                ansNum++;
            }
            maxEnd = Math.max(maxEnd, endStartFirst[i][1]);
        }
        temp[ansNum][0] = minStart;
        temp[ansNum][1] = maxEnd;
        ansNum++;
    ...
    public int[][] mergeForSortByEndStart(int[][] leftArr, int[][] rightArr) {
        ...
        while (leftPoint < leftArr.length && rightPoint < rightArr.length) {
            ...
            while (leftPoint < leftArr.length && rightPoint < rightArr.length
                && leftArr[leftPoint][0] == rightArr[rightPoint][0]) {
                // 距离最长
                if (leftArr[leftPoint][1] >= rightArr[rightPoint][1]) {
                    temp[currentPoint] = leftArr[leftPoint];
                    leftPoint++;
                } else {
