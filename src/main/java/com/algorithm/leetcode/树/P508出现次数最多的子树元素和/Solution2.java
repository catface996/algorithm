package com.algorithm.leetcode.树.P508出现次数最多的子树元素和;
...
class Solution2 {
    ...
    public int[] findFrequentTreeSum(TreeNode root) {
        ...
        8:53 下午	info
				解答成功:
				执行耗时:4 ms,击败了93.28% 的Java用户
				内存消耗:38.7 MB,击败了73.75% 的Java用户
         */
        if (root == null) {
            return new int[0];
        }
    ...
    private int process(TreeNode x) {
        ...
        int count = subSumCount.getOrDefault(subSum, 0) + 1;
        subSumCount.put(subSum, count);
