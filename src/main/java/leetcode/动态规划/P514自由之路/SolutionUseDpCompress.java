...
public class SolutionUseDpCompress {
    ...
    //3:08 下午	info
    //				解答成功:
    //				执行耗时:62 ms,击败了7.69% 的Java用户
    //				内存消耗:38.9 MB,击败了52.22% 的Java用户
     ...
     */
    private int process() {
        int[][] dp = new int[ringSize][2];
        int target = 0;
        int source = 1;
        for (int currentIndexOnKey = key.length() - 1; currentIndexOnKey >= 0; currentIndexOnKey--) {
            char targetChar = key.charAt(currentIndexOnKey);
            for (int currentIndexOnRing = ringSize - 1; currentIndexOnRing >= 0; currentIndexOnRing--) {
                ...
                for (Integer targetCharOnRingIndex : targetCharOnRingIndexList) {
                    ...
                    int rightTotalCost = rightCost + 1 + dp[targetCharOnRingIndex][source];
                    int leftCost = Math.abs(ringSize - rightCost);
                    int leftTotalCost = leftCost + 1 + dp[targetCharOnRingIndex][source];
                    minStep = Math.min(minStep, rightTotalCost);
                    minStep = Math.min(minStep, leftTotalCost);
                }
                dp[currentIndexOnRing][target] = minStep;
            }
    ...
    public static class TestClass {
        ...
        public void worst() {
            ...
            for (int size = 0; size <= 600; ) {
                size += 100;
                for (int i = 0; i < size; i++) {
                    sb.append('A');
                }
                String ring = sb.toString();
                String key = sb.toString();
                SolutionUseDp solutionUseDp = new SolutionUseDp();
                SolutionUseCache solutionUseCache = new SolutionUseCache();

                long start2 = System.currentTimeMillis();
                int ans2 = solutionUseDp.findRotateSteps(ring, key);
                long start3 = System.currentTimeMillis();
                int ans3 = solutionUseCache.findRotateSteps(ring, key);
                long start4 = System.currentTimeMillis();
                long useDpTime = start3 - start2;
                long useCacheTime = start4 - start3;
                log.info("useCache:{},useDp:{},times:{}", useCacheTime, useDpTime, 1.0 * useCacheTime / useDpTime);
                assert ans2 == ans3;
            }
