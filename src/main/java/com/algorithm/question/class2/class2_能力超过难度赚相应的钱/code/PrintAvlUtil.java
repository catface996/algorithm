...
import com.algorithm.question.class2.class2_能力超过难度赚相应的钱.code.Solution3.Node;
...
public class PrintAvlUtil {
     ...
     * @return 二维矩阵
     */
    private static char[][] initTreeBitmap(Node x) {
        int nodeWidth = 7;
     ...
     * @return 最大深度
     */
    private static int getMaxDepth(Node x) {
        if (x == null) {
            return 0;
        }
     ...
     */
    private static void process(Node x, int start, int end, int depth, char[][] bitmap) {
        if (x == null) {
            return;
        }
